    ...
    if (isEmpty(username, password, role)) {
        ...
        try {
            ...
            if (loggedInUser && loggedInUser.status === 0) {
                ...
                // to verify end-user role matches db role, (i.e., Customer or Admin for now.)
                else if (!loggedInUser.role.includes(role.toLowerCase())) res.status(status_codes.auth).send(responseFunction(true, status_codes.auth, "You do not have access to the page you requested."))
                // once all validations are successfully valid then generate token.
                else {
